...
def create_hours_chart(stats_data: List[Dict[str, Any]], period_label: str) -> io.BytesIO:
    ...
    ВАЖНО: Применяется смещение времени на основе часового пояса из config.TIMEZONE.
    Данные в базе могут храниться в UTC или другом часовом поясе, поэтому для корректного
    отображения времени на графике применяется смещение. Например, для Europe/Moscow
    (UTC+3) час 10:00 из базы данных будет отображен как 13:00 на графике.
    
    Args:
        stats_data: Список словарей с полями hour (0-23) и count
                   (часы в исходном часовом поясе из базы данных)
        period_label: Подпись периода для заголовка
    
    Returns:
        BytesIO: Изображение графика в байтах
    """
    # КОСТЫЛЬ: Вычисляем фиксированное смещение часового пояса для корректного отображения времени на графике
    # Проблема: данные в базе могут храниться в UTC или другом часовом поясе,
    # а на графике нужно показывать время в часовом поясе из config.TIMEZONE.
    # Решение: вычисляем смещение для стандартного времени (без учета летнего времени) и применяем его к часам.
    # Например, если TIMEZONE = "Europe/Moscow" (UTC+3), то час 10:00 из базы
    # будет отображен как 13:00 на графике.
    #
    # Используем фиксированную дату в зимнее время для получения стандартного смещения (без DST)
    tz = pytz.timezone(config.TIMEZONE)
    # Используем январь (зимнее время) для получения стандартного смещения без учета DST
    winter_date = datetime(2024, 1, 1, 12, 0, 0, tzinfo=pytz.UTC)
    winter_local = winter_date.astimezone(tz)
    timezone_offset_hours = int((winter_local - winter_date).total_seconds() // 3600)
    
    logger.debug(f"Applying fixed timezone offset: {timezone_offset_hours} hours for timezone {config.TIMEZONE} (without DST)")
    
    # Применяем смещение к данным: перераспределяем статистику по часам с учетом смещения
    # Создаем новый словарь для перераспределенных данных (24 часа)
    shifted_stats = {h: 0 for h in range(24)}
    
    for item in stats_data:
        original_hour = item['hour']
        count = item['count']
        # Вычисляем новый час с учетом смещения (по модулю 24 для циклического сдвига)
        # Например: если original_hour=10 и offset=3, то shifted_hour=13
        shifted_hour = (original_hour + timezone_offset_hours) % 24
        shifted_stats[shifted_hour] += count
    
    # Преобразуем обратно в список для совместимости
    hours = list(range(24))
    counts = [shifted_stats[h] for h in hours]
    ...
    # Отображаем часы с учетом смещения часового пояса
